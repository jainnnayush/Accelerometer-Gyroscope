    ...
    private fun calculateOrientation() {
        accelerometerData?.let { acc ->
            gyroscopeData?.let { gyr ->
                ...
                val pitch = atan(acc[1].toDouble()/sqrt(pow(acc[0].toDouble(), 2.0)+pow(acc[2].toDouble(),
                    2.0
                ))) * (180 / Math.PI)
                val roll = atan(-acc[0].toDouble()/sqrt(pow(acc[1].toDouble(), 2.0)+pow(acc[2].toDouble(),
                    2.0
                ))) * (180 / Math.PI)
                val yaw = gyr[2] * (180 / Math.PI) // Simplified placeholder
